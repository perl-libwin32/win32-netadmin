...
#define A2WHELPER_LEN(lpa, alen, lpw, nBytes)                                  \
    (lpw[0] = 0,                                                               \
     MultiByteToWideChar((IN_BYTES) ? CP_ACP : CP_UTF8,                        \
                         0,                                                    \
                         lpa,                                                  \
                         alen,                                                 \
                         lpw,                                                  \
...
#define W2AHELPER_LEN(lpw, wlen, lpa, nChars)                                  \
    (lpa[0] = '\0',                                                            \
     WideCharToMultiByte((IN_BYTES) ? CP_ACP : CP_UTF8,                        \
                         0,                                                    \
                         lpw,                                                  \
                         wlen,                                                 \
                         (LPSTR)lpa,                                           \
...
DWORD
GetAccountSIDA(LPCSTR lpSystemName, LPCSTR lpAccountName, PSID* pSid)
{
    DWORD sidSize;
    char refDomain[256];
    DWORD refDomainSize;
    ...
    sidSize = 0;
    refDomainSize = 255;
    LookupAccountNameA(lpSystemName,
                       lpAccountName,
                       *pSid,
                       &sidSize,
                       refDomain,
    ...
    *pSid = (PSID)malloc(sidSize);
    refDomainSize = 255;
    if (!LookupAccountNameA(lpSystemName,
                            lpAccountName,
                            *pSid,
                            &sidSize,
                            refDomain,
...
DWORD
GetAccountSIDW(LPCWSTR lpSystemName, LPCWSTR lpAccountName, PSID* pSid)
{
    DWORD sidSize;
    WCHAR refDomain[256];
    DWORD refDomainSize;
    ...
    sidSize = 0;
    refDomainSize = 255;
    LookupAccountNameW(lpSystemName,
                       lpAccountName,
                       *pSid,
                       &sidSize,
                       refDomain,
    ...
    *pSid = (PSID)malloc(sidSize);
    refDomainSize = 255;
    if (!LookupAccountNameW(lpSystemName,
                            lpAccountName,
                            *pSid,
                            &sidSize,
                            refDomain,
...
static void
SetNetAdminLastError(DWORD dwError)
{
...
XS(XS_NT__NetAdmin_GetDomainController)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::GetDomainController(server, domain, "
              "returnedName)\n");
    }
    {
        STRLEN n_a;
...
XS(XS_NT__NetAdmin_GetAnyDomainController)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::GetAnyDomainController(server, domain, "
              "returnedName)\n");
    }
    {
        STRLEN n_a;
...
XS(XS_NT__NetAdmin_UserSetAttributes)
{
    ...
    }
    {
        ...
        uiUser.usri1_flags = SvIV(ST(7));
        AllocWideName((char*)SvPV(ST(8), n_a), uiUser.usri1_script_path);
        lastError = NetUserSetInfo(
          lpwServer, uiUser.usri1_name, 1, (LPBYTE)&uiUser, NULL);
        FreeWideName(lpwServer);
        FreeWideName(uiUser.usri1_name);
        FreeWideName(uiUser.usri1_password);
        FreeWideName(uiUser.usri1_home_dir);
...
XS(XS_NT__NetAdmin_UserChangePassword)
{
    ...
    }
    {
        ...
        lastError = NetUserChangePassword(
          lpwDomain, lpwUserName, lpwOldPassword, lpwNewPassword);
        FreeWideName(lpwDomain);
...
XS(XS_NT__NetAdmin_GetUsers)
{
    ...
    }
    {
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            AllocWideName((char*)SvPV(ST(0), n_a), lpwServer);
            do {
                lastError = NetUserEnum(lpwServer,
                                        0,
                                        filter,
                                        (LPBYTE*)&pwzUsers,
                                        PREFLEN,
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(
                      pwzUsers[index].usri0_name, buffer, sizeof(buffer));
            ...
            do {
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(
                      pwzUsers10[index].usri10_name, buffer, sizeof(buffer));
                    W2AHELPER(pwzUsers10[index].usri10_full_name,
                              buffer1,
                              sizeof(buffer1));
                    hv_store(
                      (HV*)sv, buffer, strlen(buffer), newSVpv(buffer1, 0), 0);
                }
...
XS(XS_NT__NetAdmin_GetTransports)
{
    ...
    }
    {
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            AllocWideName((char*)SvPV(ST(0), n_a), lpwServer);
            do {
                lastError = NetWkstaTransportEnum(lpwServer,
                                                  0,
                                                  (LPBYTE*)&pws,
                                                  PREFLEN,
                                                  &entriesRead,
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(
                      pws[index].wkti0_transport_name, buffer, sizeof(buffer));
                    av_push((AV*)sv, newSVpv(buffer, 0));
                }
            ...
            do {
                ...
                for (index = 0; index < entriesRead; ++index) {
                    ...
                    hv_store(hvTemp,
                             ...
                             newSViv((long)pws[index].wkti0_quality_of_service),
                             0);
                    hv_store(hvTemp,
                             "number_of_vcs",
                             strlen("number_of_vcs"),
                             newSViv((long)pws[index].wkti0_number_of_vcs),
                             0);
                    W2AHELPER(
                      pws[index].wkti0_transport_name, buffer, sizeof(buffer));
                    hv_store(hvTemp,
                             ...
                             newSVpv((char*)buffer, strlen((char*)buffer)),
                             0);
                    W2AHELPER(pws[index].wkti0_transport_address,
                              buffer1,
                              sizeof(buffer1));
                    hv_store(hvTemp,
                             "transport_address",
                             strlen("transport_address"),
                             newSVpv((char*)buffer1, strlen((char*)buffer1)),
                             0);
                    hv_store(hvTemp,
                             "wan_ish",
                             strlen("wan_ish"),
                             newSViv((bool)pws[index].wkti0_wan_ish),
                             0);
...
XS(XS_NT__NetAdmin_LoggedOnUsers)
{
    ...
    }
    {
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            AllocWideName((char*)SvPV(ST(0), n_a), lpwServer);
            do {
                lastError = NetWkstaUserEnum(lpwServer,
                                             0,
                                             (LPBYTE*)&pwzUser0,
                                             PREFLEN,
                                             &entriesRead,
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(
                      pwzUser0[index].wkui0_username, buffer, sizeof(buffer));
                    av_push((AV*)sv, newSVpv(buffer, 0));
                }
            ...
            do {
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(
                      pwzUser1[index].wkui1_username, buffer, sizeof(buffer));
                    W2AHELPER(pwzUser1[index].wkui1_logon_domain,
                              logon_domain,
                              sizeof(logon_domain));
                    W2AHELPER(pwzUser1[index].wkui1_logon_server,
                              logon_server,
                              sizeof(logon_server));
                    sprintf(
                      buffer1, "%s;%s;%s", buffer, logon_domain, logon_server);
...
XS(XS_NT__NetAdmin_GroupGetAttributes)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::GroupGetAttributes(server, groupName, "
              "comment)\n");
    }
    {
        STRLEN n_a;
        ...
        AllocWideName((char*)SvPV(ST(1), n_a), lpwGroup);
        lastError =
          NetGroupGetInfo(lpwServer, lpwGroup, 2, (LPBYTE*)&pgroupInfo);
        if (lastError == 0) {
...
XS(XS_NT__NetAdmin_GroupSetAttributes)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::GroupSetAttributes(server, groupName, "
              "comment)\n");
    }
    {
        STRLEN n_a;
        ...
        lastError = NetGroupSetInfo(
          lpwServer, groupInfo.grpi2_name, 2, (LPBYTE)&groupInfo, NULL);
        FreeWideName(lpwServer);
...
XS(XS_NT__NetAdmin_GroupAddUsers)
{
    ...
    if (items != 3) {
        croak(
          "Usage: Win32::NetAdmin::GroupAddUsers(server, groupName, users)\n");
    }
    {
        STRLEN n_a;
        AllocWideName((char*)SvPV(ST(0), n_a), lpwServer);
        ...
        switch (SvTYPE(sv)) {
                ...
                for (index = 0; index <= count; ++index) {
                    ...
                    if (psv != NULL) {
                        MultiByteToWideChar(CP_ACP,
                                            ...
                                            sizeof(wzUser));
                        lastError =
                          NetGroupAddUser(lpwServer, lpwGroup, wzUser);
                        if (lastError != 0)
                            break;
                    }
                }
            ...
            default:
                MultiByteToWideChar(
                  CP_ACP, 0, (char*)SvPV(sv, n_a), -1, wzUser, sizeof(wzUser));
...
XS(XS_NT__NetAdmin_GroupDeleteUsers)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::GroupDeleteUsers(server, groupName, "
              "users)\n");
    }
    {
        STRLEN n_a;
                ...
                for (index = 0; index <= count; ++index) {
                    ...
                    if (psv != NULL) {
                        MultiByteToWideChar(CP_ACP,
                                            ...
                                            sizeof(wzUser));
                        lastError =
                          NetGroupDelUser(lpwServer, lpwGroup, wzUser);
                        if (lastError != 0)
                            break;
                    }
                }
...
XS(XS_NT__NetAdmin_GroupIsMember)
{
    ...
    if (items != 3) {
        croak(
          "Usage: Win32::NetAdmin::GroupIsMember(server, groupName, user)\n");
    }
    {
        STRLEN n_a;
        AllocWideName((char*)SvPV(ST(0), n_a), lpwServer);
...
#else
        lastError = NetUserGetGroups(lpwServer,
                                     lpwUser,
                                     0,
                                     (LPBYTE*)&pwzGroupUsers,
                                     PREFLEN,
...
XS(XS_NT__NetAdmin_GroupGetMembers)
{
    ...
    }
    {
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            AllocWideName((char*)SvPV(ST(1), n_a), lpwGroup);
            do {
                lastError = NetGroupGetUsers(lpwServer,
                                             lpwGroup,
                                             0,
                                             (LPBYTE*)&pwzGroupUsers,
                                             PREFLEN,
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(
                      pwzGroupUsers[index].grui0_name, buffer, sizeof(buffer));
                    av_push((AV*)sv, newSVpv(buffer, 0));
                }
                NetApiBufferFree(pwzGroupUsers);
            } while (lastError == ERROR_MORE_DATA);
...
XS(XS_NT__NetAdmin_LocalGroupCreate)
{
    ...
    if (items != 3) {
        croak(
          "Usage: Win32::NetAdmin::LocalGroupCreate(server, group, comment)\n");
    }
    {
        STRLEN n_a;
        AllocWideName((char*)SvPV(ST(0), n_a), lpwServer);
...
XS(XS_NT__NetAdmin_LocalGroupGetAttributes)
{
    ...
    if (items != 3) {
        croak(
          "Usage: Win32::NetAdmin::LocalGroupGetAttributes(server, groupName, "
          "comment)\n");
    }
    {
        STRLEN n_a;
        ...
        AllocWideName((char*)SvPV(ST(1), n_a), lpwGroup);
        lastError =
          NetLocalGroupGetInfo(lpwServer, lpwGroup, 1, (LPBYTE*)&pgroupInfo);
        if (lastError == 0) {
...
XS(XS_NT__NetAdmin_LocalGroupSetAttributes)
{
    ...
    if (items != 3) {
        croak(
          "Usage: Win32::NetAdmin::LocalGroupSetAttributes(server, groupName, "
          "comment)\n");
    }
    {
        STRLEN n_a;
        ...
        AllocWideName((char*)SvPV(ST(1), n_a), groupInfo.lgrpi1_name);
        AllocWideName((char*)SvPV(ST(2), n_a), groupInfo.lgrpi1_comment);
        lastError = NetLocalGroupSetInfo(
          lpwServer, groupInfo.lgrpi1_name, 1, (LPBYTE)&groupInfo, NULL);
        FreeWideName(lpwServer);
        FreeWideName(groupInfo.lgrpi1_name);
        FreeWideName(groupInfo.lgrpi1_comment);
    }
...
XS(XS_NT__NetAdmin_LocalGroupAddUsers)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::LocalGroupAddUsers(server, groupName, "
              "users)\n");
    }
    {
        STRLEN n_a;
                ...
                for (index = 0; index <= count; ++index) {
                    ...
                    if (psv != NULL) {
                        MultiByteToWideChar(CP_ACP,
                                            ...
                                            wzUser,
                                            sizeof(wzUser));
                        lastError = NetLocalGroupAddMembers(
                          lpwServer, lpwGroup, 3, (LPBYTE)&lgmi3MembersInfo, 1);
                        if (lastError != 0)
                            break;
                    }
                }
...
XS(XS_NT__NetAdmin_LocalGroupDeleteUsers)
{
    ...
    if (items != 3) {
        croak(
          "Usage: Win32::NetAdmin::LocalGroupDeleteUsers(server, groupName, "
          "users)\n");
    }
    {
        STRLEN n_a;
                ...
                for (index = 0; index <= count; ++index) {
                    ...
                    if (psv != NULL) {
                        MultiByteToWideChar(CP_ACP,
                                            ...
                                            wzUser,
                                            sizeof(wzUser));
                        lastError = NetLocalGroupDelMembers(
                          lpwServer, lpwGroup, 3, (LPBYTE)&lgmi3MembersInfo, 1);
                        if (lastError != 0)
                            break;
                    }
...
XS(XS_NT__NetAdmin_LocalGroupIsMember)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::LocalGroupIsMember(server, groupName, "
              "user)\n");
    }
    {
        STRLEN n_a;
#if 1
        PSID pSid;
        lastError = GetAccountSIDA(
          (LPCTSTR)SvPV(ST(0), n_a), (LPCTSTR)SvPV(ST(2), n_a), &pSid);
        ...
        do {
            ...
            lastError = NetLocalGroupGetMembers(lpwServer,
                                                lpwGroup,
                                                0,
        ...
        AllocWideName((char*)SvPV(ST(2), n_a), lpwUser);
        {
            ...
            lastError = NetUserGetLocalGroups(lpwServer,
                                              lpwUser,
                                              0,
                                              LG_INCLUDE_INDIRECT,
                                              (LPBYTE*)&pwzGroupUsers,
...
XS(XS_NT__NetAdmin_LocalGroupGetMembers)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::LocalGroupGetMembers(server, groupName, "
              "\\@userArray)\n");
    }
    {
        STRLEN n_a;
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            do {
                ...
                if (lastError != 0 && lastError != ERROR_MORE_DATA)
                    break;
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(pwzMembersInfo[index].lgrmi1_name,
                              buffer,
                              sizeof(buffer));
                    av_push((AV*)sv, newSVpv(buffer, 0));
                }
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            do {
                ...
                if (lastError != 0 && lastError != ERROR_MORE_DATA)
                    break;
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(pwzMembersInfo[index].lgrmi2_domainandname,
                              buffer,
                              sizeof(buffer));
                    av_push((AV*)sv, newSVpv(buffer, 0));
                }
            ...
            do {
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(pwzMembersInfo[index].lgrmi2_domainandname,
                              ...
                              sizeof(buffer));
                    sprintf(
                      buffer1, "%d", pwzMembersInfo[index].lgrmi2_sidusage);
                    hv_store(
                      (HV*)sv,
                      buffer,
                      strlen(buffer),
                      newSViv((DWORD)pwzMembersInfo[index].lgrmi2_sidusage),
                      0);
                }
                NetApiBufferFree(pwzMembersInfo);
            } while (lastError == ERROR_MORE_DATA || resumeHandle != 0);
            ...
            croak(
              "Usage: Win32::NetAdmin::LocalGroupGetMembersWithDomain(server, "
              "groupName, userRef)\nuserRef was not an array or hash "
              "reference\n");
        }
    }
    SetNetAdminLastError(lastError);
...
XS(XS_NT__NetAdmin_GetServers)
{
    ...
    }
    {
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            AllocWideName((char*)SvPV(ST(1), n_a), lpwDomain);
            do {
                lastError = NetServerEnum(lpwServer,
                                          100,
                ...
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(
                      pwzServerInfo[index].sv100_name, buffer, sizeof(buffer));
                    av_push((AV*)sv, newSVpv(buffer, 0));
                }
            ...
            do {
                ...
                if (lastError != 0 && lastError != ERROR_MORE_DATA)
                    break;
                for (index = 0; index < entriesRead; ++index) {
                    W2AHELPER(pwzServerInfo101[index].sv101_name,
                              buffer,
                              sizeof(buffer));
                    W2AHELPER(pwzServerInfo101[index].sv101_comment,
                              buffer1,
                              sizeof(buffer1));
                    hv_store(
                      (HV*)sv, buffer, strlen(buffer), newSVpv(buffer1, 0), 0);
...
XS(XS_NT__NetAdmin_GetServerDisks)
{
    ...
    }
    {
        ...
        if (SvTYPE(sv) == SVt_PVAV) {
            ...
            AllocWideName((char*)SvPV(ST(0), n_a), lpwServer);
            do {
                lastError = NetServerDiskEnum(lpwServer,
                                              0,
                                              (LPBYTE*)&disks,
                                              PREFLEN,
                                              &entriesRead,
...
XS(XS_NT__NetAdmin_GetAliasFromRID)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::GetAliasFromRID(server, RID, "
              "returnedName)\n");
    }
    {
        STRLEN n_a;
...
XS(XS_NT__NetAdmin_GetUserGroupFromRID)
{
    ...
    if (items != 3) {
        croak("Usage: Win32::NetAdmin::GetUserGroupFromRID(server, RID, "
              "returnedName)\n");
    }
    {
        STRLEN n_a;
...
XS(boot_Win32__NetAdmin)
{
    ...
    newXS("Win32::NetAdmin::GetDomainController",
          XS_NT__NetAdmin_GetDomainController,
          file);
    newXS("Win32::NetAdmin::GetAnyDomainController",
          XS_NT__NetAdmin_GetAnyDomainController,
          file);
    ...
    newXS("Win32::NetAdmin::UserGetAttributes",
          XS_NT__NetAdmin_UserGetAttributes,
          file);
    newXS("Win32::NetAdmin::UserSetAttributes",
          XS_NT__NetAdmin_UserSetAttributes,
          file);
    ...
    newXS(
      "Win32::NetAdmin::LoggedOnUsers", XS_NT__NetAdmin_LoggedOnUsers, file);
    ...
    newXS("Win32::NetAdmin::GroupGetAttributes",
          XS_NT__NetAdmin_GroupGetAttributes,
          file);
    newXS("Win32::NetAdmin::GroupSetAttributes",
          XS_NT__NetAdmin_GroupSetAttributes,
          file);
    newXS(
      "Win32::NetAdmin::GroupAddUsers", XS_NT__NetAdmin_GroupAddUsers, file);
    newXS("Win32::NetAdmin::GroupDeleteUsers",
          XS_NT__NetAdmin_GroupDeleteUsers,
          file);
    newXS(
      "Win32::NetAdmin::GroupIsMember", XS_NT__NetAdmin_GroupIsMember, file);
    newXS("Win32::NetAdmin::GroupGetMembers",
          XS_NT__NetAdmin_GroupGetMembers,
          file);
    newXS("Win32::NetAdmin::LocalGroupCreate",
          XS_NT__NetAdmin_LocalGroupCreate,
          file);
    newXS("Win32::NetAdmin::LocalGroupDelete",
          XS_NT__NetAdmin_LocalGroupDelete,
          file);
    newXS("Win32::NetAdmin::LocalGroupGetAttributes",
          XS_NT__NetAdmin_LocalGroupGetAttributes,
          file);
    newXS("Win32::NetAdmin::LocalGroupSetAttributes",
          XS_NT__NetAdmin_LocalGroupSetAttributes,
          file);

    newXS("Win32::NetAdmin::LocalGroupAddUsers",
          XS_NT__NetAdmin_LocalGroupAddUsers,
          file);
    newXS("Win32::NetAdmin::LocalGroupDeleteUsers",
          XS_NT__NetAdmin_LocalGroupDeleteUsers,
          file);
    newXS("Win32::NetAdmin::UserChangePassword",
          XS_NT__NetAdmin_UserChangePassword,
          file);

    newXS("Win32::NetAdmin::LocalGroupIsMember",
          XS_NT__NetAdmin_LocalGroupIsMember,
          file);
    newXS("Win32::NetAdmin::LocalGroupGetMembers",
          XS_NT__NetAdmin_LocalGroupGetMembers,
          file);
    newXS("Win32::NetAdmin::LocalGroupGetMembersWithDomain",
          XS_NT__NetAdmin_LocalGroupGetMembersWithDomain,
          file);
    newXS("Win32::NetAdmin::GetServers", XS_NT__NetAdmin_GetServers, file);
    newXS(
      "Win32::NetAdmin::GetServerDisks", XS_NT__NetAdmin_GetServerDisks, file);
    newXS(
      "Win32::NetAdmin::GetTransports", XS_NT__NetAdmin_GetTransports, file);
    newXS("Win32::NetAdmin::GetAliasFromRID",
          XS_NT__NetAdmin_GetAliasFromRID,
          file);
    newXS("Win32::NetAdmin::GetUserGroupFromRID",
          XS_NT__NetAdmin_GetUserGroupFromRID,
          file);
    XSRETURN_YES;
}
